...
def run_backtest(df, weights, sip_amount):
    ...
    Returns: Dictionary of metrics (XIRR, RoMaD, etc.) and the Result DataFrame
    """
    ...
    for date, row in monthly_data.iterrows():
        ...
        for f in weights.keys():
            if row[f] > 0:
                # weights[f] is percent (e.g. 50), need to divide by 100
    ...
    # Sharpe (Estimate based on monthly XIRR path implied volatility is hard for SIP)
    # We use daily returns of the underlying index mix for Sharpe
    daily_ret = df.pct_change().dropna()
    # Normalize weights to 0-1 range for dot product
                    ...
                    if max_yrs < 0.1:
                        ...
                        with col_chart:
                            ...
                            if m_metrics:
                                ...
                                mc2.metric("RoMaD", f"{m_metrics['RoMaD']:.2f}", help=f"Return per unit of Crash. (Max DD: {m_metrics['MaxDD']:.1f}%)")
                                mc3.metric("Sharpe", f"{m_metrics['Sharpe']:.2f}", help="Return per unit of Volatility")
